...
-- Enable RLS on check_history（Realtime postgres_changes 依赖 SELECT policy）
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime' AND schemaname = 'dev' AND tablename = 'check_history'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE dev.check_history;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'dev' AND tablename = 'group_info'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE dev.group_info;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'dev' AND tablename = 'system_notifications'
  ) THEN
